...
class Resolver extends Collection {
  ...
  getUsage (usage = this.usage, { prefix, command } = {}) {
    const argsUsage = usage.map(arg => {
      let display = arg.displayName
      if (arg.type === 'string' && Array.isArray(arg.choices)) {
        display = arg.choices.join(' | ')
      }
      return arg.last ? display : arg.optional ? `[${display}]` : `<${display}>`
    }).join(' ')
